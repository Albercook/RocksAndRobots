...
int basespeed = 160;  //#Declare variable: Startup Power setting. Using value from ***Motor Test Experiment
int helispeed = 135; 
int helispeed7 = 135;
int helispeed8 = 135;
...
void loop()  { 
  ...
  while(1){
    ...
    int gyro_sensor_val = analogRead(GYRO_SENSOR_PIN);
     ...
     if (scan2 == -1500 || scan2 == 1500) {
     scan2Dir = -scan2Dir ; 
     }     
    
     //yaw_rate =  0; 
     yaw_rate = (((base_gyro_val+rudder) - gyro_sensor_val)/2); 
     //yaw_rate = (((base_gyro_val+rudder/3+scan/2) - gyro_sensor_val)/2); 
     //yaw_rate = (((base_gyro_val+rudder/3+scan2/50) - gyro_sensor_val/2)); 
     //yaw_rate = (((base_gyro_val+rudder+scan/2+scan2/30) - gyro_sensor_val)/2); 
    ...
    if (counter > 5000){
      while(1){
        analogWrite(motor1, normhelispeed7); 
        analogWrite(motor2, normhelispeed8);           
        normhelispeed7 = int(long(9)*(long(normhelispeed7)/long(10)));
        normhelispeed8 = int(long(9)*(long(normhelispeed8)/long(10)));
        delay(200);  
      }  
    }
    if (VCCmV < 2300){
